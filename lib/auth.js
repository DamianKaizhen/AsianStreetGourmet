...
// ----------------------------------------------------------------------
// Active password hash lookup.
//
// The hash lives in the `settings` table under key `admin_password_hash`
// once the admin has changed it from the UI. Until then we fall back to
// the bootstrap env var `ADMIN_PASSWORD_HASH`. Note: once the DB row
// exists, the env var is IGNORED — editing it on Vercel will have no
// effect until you `DELETE FROM settings WHERE key='admin_password_hash'`
// in the Neon SQL editor (the documented lockout-recovery path).
// ----------------------------------------------------------------------

export async function getActivePasswordHash() {
  try {
    // Lazy import so this module stays dep-free at load time — `settings.js`
    // pulls in the Neon client which we don't want to hard-require here.
    ...
    // DB blip — fall back to env so login keeps working.
...
//
// Returns { ok, recovery } where:
//   ok       — true when credentials match
//   recovery — true when this login succeeded via the env-var fallback
//              because ADMIN_RECOVERY_MODE was set. The caller (api/auth.js)
//              surfaces this to the UI so the family is reminded to set a
//              new password and remove the recovery env var.
//
// Recovery mode (`process.env.ADMIN_RECOVERY_MODE` set to anything truthy):
//   - the DB row (if any) is BYPASSED
//   - only the env-var hash (`ADMIN_PASSWORD_HASH`) is accepted
//   - on successful login the DB row is wiped, so the env var becomes the
//     live password again. After that, the family can use the in-UI
//     "Change password" form to set whatever they want, and remove
//     ADMIN_RECOVERY_MODE from Vercel.
// ----------------------------------------------------------------------
...
export async function checkAdminCredentials(username, password) {
  ...
  const expectedHash = recovery
    ? (process.env.ADMIN_PASSWORD_HASH || '')
    : ((await getActivePasswordHash()) || '');
  if (!expectedUser || !expectedHash) return { ok: false, recovery: false };
  ...
  // Recovery: on success, wipe the DB row so the env-var hash is the
  // active password again. The family is then expected to remove
  // ADMIN_RECOVERY_MODE from Vercel and (optionally) set a new password.
  if (ok && recovery) {
    try {
  ...
  return { ok, recovery: ok && recovery };
}
