...
/**
 * NY-time day-of-week (0=Sun..6=Sat). Matches the format soup_schedule
 * uses. Uses Intl rather than raw new Date().getDay() so orders placed
 * from any client TZ or server pod resolve to the New York calendar day.
 */
function nyDayOfWeek(now = new Date()) {
...
/**
 * Which soup is scheduled for the current NY-time day. Returns null if
 * no soup is scheduled (no row in soup_schedule for that day) — callers
 * treat null as "no soup can be ordered today".
 */
async function getTodaysSoupCode() {
  try {
    const rows = await sql`
      SELECT soup_code FROM soup_schedule
      WHERE day_of_week = ${nyDayOfWeek()}
      LIMIT 1
    `;
    return (rows[0] && rows[0].soup_code) || null;
  } catch (err) {
    // If the schedule table is unreachable, fall through and let the
    // "no soup allowed today" branch fire — safer than silently allowing
    // any soup when we can't verify.
    console.warn('[orders] soup_schedule read failed:', err.message);
    return null;
  }
}
...
export async function resolveCartItems(items, { allowUnavailable = false } = {}) {
  ...
  // requested, so this stays a 3-roundtrip resolve no matter the cart size.
  // The third roundtrip pulls the current day's scheduled soup code so we
  // can reject any soup that isn't the "soup of the day" — the family
  // only cooks one soup on any given day, and the site would otherwise
  // silently accept an order for a soup they can't fulfill.
  const [menuRows, specialsRaw, soupTodayCode] = await Promise.all([
    sql`
  ...
  for (const r of requested) {
    ...
    // Soup-of-the-day gate: the family only cooks one soup per day, so
    // any soup that isn't the currently-scheduled one gets treated as
    // "unavailable today". allowUnavailable doesn't bypass this — even
    // the POS shouldn't ring a soup the kitchen isn't cooking (the
    // cashier can update the day's soup_schedule from admin first).
    if (row.category === 'soup' && row.code !== soupTodayCode) {
      unavailable.push(r.code);
      continue;
    }
    if (!row.is_available) {
